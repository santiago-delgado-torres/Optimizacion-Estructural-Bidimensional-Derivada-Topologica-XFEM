...
while abs(Omega/Omega0-M)>TolM
   ...
   % en esta iteracion.
    ...
    if DeltaVolMax<TolDeltaV % La estructura ya es muy optima

       ES.psi(~ES.Must)=psiOld;
       ES=interfase(ES);
       Omega=VolumenXFEM(ES);
       while DeltaVolMax<TolDeltaV
       ...
       while Fnew > Fold && sit<MaxSit
           ...
           axis equal
           set(gca,'LooseInset',max(get(gca,'TightInset'),0.02))
           title(['Iteracion ',num2str(n), '; Subiteracion ',num2str(sit),'; Volumen ',num2str(100*Omega/Omega0),'% Lambda: ',num2str(Lambda),'; \Delta Vmax: ',num2str(100*DeltaVolMax),'%; F_{old}: ',num2str(Fold),'; F_{new}: ',num2str(Fnew)])
           pause(0)
       ...
       if SmoothingCoef>0
           % Paso de suavizar
           if ES.Nnodo == (ES.Nx+1)*(ES.Ny+1) %Caso sencillo y rapido, todo el rectangulo tiene elementos
       ...
       ylim([min(ES.Mnodo(:,3)) max(ES.Mnodo(:,3))])
       set(gca,'LooseInset',max(get(gca,'TightInset'),0.02))
       title(['Iteracion ',num2str(n), '; Volumen ',num2str(100*Omega/Omega0),'% \lambda: ',num2str(Lambda)])
       ...
       if rem(n,10)==0 %El numero de iteraciones coincide con este valor
